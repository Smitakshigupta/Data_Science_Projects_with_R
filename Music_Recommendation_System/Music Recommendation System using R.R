...
# Algorithms used :  Gradient Boosting Decision Tree
#Author : Smitakshi Gupta
#Kaggle Link to the data : https://www.kaggle.com/c/kkbox-music-recommendation-challenge


#Installing packages

install.packages("reshape")
library(reshape)#transforming the data into the matrix form
library(reshape2)
install.packages("xgboost") #building the boosting model
library(xgboost)
library(caret)#reading,indexing,slicing and filtering
library(jsonlite)
library(dplyr)
install.packages("Matrix")
library(Matrix)
install.packages("doParallel")# performing parallel operations
library(doParallel)
library(lubridate)

#getwd()

setwd("/Users/smitakshigupta/..../Music Recommendation System")

#Importing the train data
train <- read.csv("train.csv",sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?"))
test <- read.csv("test.csv",sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?"))

# Explore data:

train$key <-paste(train$msno,train$song_id,sep="-")
test$key <- paste(test$msno,train$song_id,sep="-")
...
#Merging the train and the test by rows
...
#Transforming the variable into 0 and 1 
...
#k-fold cross-validation :2 fold,30 iterations
